...
is_interesting_master_event({_, tap_estimate, _, _}) -> true;
is_interesting_master_event(_) -> false.
...
initiate_bucket_rebalance(BucketName) ->
    ...
    BuildDestinations1 = [{N, VB} || {VB, _, ChainAfter} <- Diff,
                                     N <- ChainAfter],
    BuildDestinations = misc:keygroup(1, lists:merge(lists:sort(BuildDestinations0),
                                                     lists:sort(BuildDestinations1))),
    SomeEstimates = misc:parallel_map(
                       fun ({Node, VBs}) ->
    ...
    Moves =
        [begin
             {_, MasterEstimate} = lists:keyfind(MasterNode, 1, SomeEstimates),
             RBStats =
                 [begin
                      {_, ReplicaEstimate} = lists:keyfind(Replica, 1, SomeEstimates),
...
handle_cast({note, {_, set_ff_map, BucketName, _Diff}}, _State) ->
    {noreply, initiate_bucket_rebalance(BucketName)};
handle_cast({note, {_, vbucket_move_start, _Pid, _BucketName, _Node, VBucketId, _, _} = Ev}, State) ->
...
handle_cast({note, {_, tap_estimate, {_Type, _BucketName, VBucket, _Src, Dst}, Estimate, _Pid, TapName}},
            State) ->
    ...
    {noreply, State3};
handle_cast({node, {_, vbucket_move_done, _BucketName, VBucket} = Ev}, State) ->
...
docs_left_updater_init(Parent) ->
    {ok, _} = timer:send_interval(?DOCS_LEFT_REFRESH_INTERVAL, refresh),
