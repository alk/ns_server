...
generate_map(Map, Nodes, Options) ->
    ...
    ?log_info("Natural map score: ~p", [element(2, NaturalMapScore)]),
    ...
    ?log_info("Rnd maps scores: ~p, ~p", [S || {_, S} <- AllRndMapScores]),
    ...
    ?log_info("Considering ~p maps:~n~p", [length(AllMaps), [S || {_, S} <- AllMaps]]),
    ...
    ?log_info("Best map score: ~p (~p,~p,~p)", [element(2, BestMapScore), (BestMap =:= NaturalMap), (BestMap =:= RndMap1), (BestMap =:= RndMap2)]),
    BestMap.
...
do_replace_nodes_rebalance_trial(NodesCount, RemoveIndexes, AddIndexes, VBucketCount, ReplicaCount) ->
    ...
    ?log_info("FirstMap:~n~p~nReplaceMap:~n~p~n", [FirstMap, ReplaceMap]),
    ...
    ?log_info("AddedNodesSignature0:~n~p~n", [AddedNodesSignature0]),
    RemovedNodesSignature0 = [{N, master_vbucket_signature(FirstMap, N)} || N <- RemovedNodes],
    ?log_info("RemovedNodesSignature0:~n~p~n", [RemovedNodesSignature0]),
    ...
    ?log_info("Discovered mapping: ~p~n", [Mapping]),
