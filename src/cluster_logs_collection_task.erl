...
%% called remotely
-export([start_collection_per_node/2]).

-export([start_upload_per_node/3]).
...
run_node_collection(Node, BaseURL, TimestampS) ->
    misc:executing_on_new_process(
      fun () ->
              erlang:process_flag(trap_exit, true),
              do_run_node_collection(Node, BaseURL, TimestampS)
      end).
...
do_run_node_collection(Node, BaseURL, TimestampS) ->
    ...
    ?log_debug("Starting collection task for: ~p", [{Node, BaseURL, TimestampS}]),
    ...
    case wait_child(P, Node, collection) of
        ok ->
            ...
            maybe_upload_node_result(Node, Path, BaseURL);
        Err ->
            Err
    end.

maybe_upload_node_result(_Node, _Path, false) ->
    ok;
maybe_upload_node_result(Node, Path, BaseURL) ->
    get_token(upload_throttle),
    ?log_debug("Starting upload task for: ~p", [{Node, Path, BaseURL}]),
...
start_collection_per_node(TimestampS, Parent) ->
    ...
    ?log_debug("spawning collectinfo into `~s'", [Filename]),
    {Status, Output} =
        try
            misc:run_external_tool(path_config:component_path(bin, "cbcollect_info"),
                                   ["--watch-stdin", "--single-node-diag",
                                    "--initargs=" ++ InitargsFilename,
                                    Filename], [])
        catch T:E ->
                S = erlang:get_stacktrace(),
                ?log_debug("Rethrowing: ~p", [{T, E, S}]),
                erlang:raise(T,E,S)
        end,
    ?log_debug("Done"),
    case Status of
...
start_upload_per_node(Path, BaseURL, Parent) ->
