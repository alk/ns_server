...
-spec delete_vbucket(ns_memcached_name(), vbucket_id()) ->
                            ok | mc_error().
...
-spec set_vbucket(ns_memcached_name(), vbucket_id(), vbucket_state()) ->
                         ok | mc_error().
...
-spec stats(ns_memcached_name(), binary() | string()) ->
                   {ok, [{binary(), binary()}]} | mc_error().
stats(Bucket, Key) ->
