...
handle_call({find_missing, IdRevs}, _From,
    ...
    ?xdcr_extra_debug("[xmem_srv for vb ~p]: out of ~p keys, we need to send ~p "
...
handle_call({flush_docs, DocsList}, _From,
    ...
    ?xdcr_extra_debug("[xmem_srv for vb ~p]: out of total ~p docs, "
...
load_balancer(Vb, Workers) ->
    ...
    ?xdcr_extra_debug("[xmem_srv for vb ~p]: pick up worker process (id: ~p, pid: ~p)", [Vb, Id, WorkerPid]),
    WorkerPid.
