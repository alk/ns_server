...
handle_info(return_token_please, State) ->
    ...
    case State of
        #rep_state{status = RepVBStatus,
            ...
            ?x_trace(sendingPleastStopToReader, []),
...
handle_call({worker_done, Pid}, _From,
    ...
    case Workers -- [Pid] of
        ...
        [] ->
            ...
            ?X_TRACE_IF(?x_trace(lastWorkerDone,
                                 [{numChangesLeft, VbStatus2#rep_vb_status.num_changes_left},
                                  ...
                                  {lastCheckpointTime, xdcr_trace:format_ts(State2#rep_state.last_checkpoint_time)},
                                  {lastStartTime, xdcr_trace:format_ts(State2#rep_state.rep_start_time)},
...
terminate(Reason, #init_state{rep = #rep{target = TargetRef}, parent = P, vb = Vb} = InitState) ->
    ?X_TRACE_IF(?x_trace(terminateInInitState, [{reason, xdcr_trace:format_pp(Reason)}])),
...
terminate(Reason, #rep_state{
    ...
    ?X_TRACE_IF(?x_trace(terminate, [{reason, xdcr_trace:format_pp(Reason)}])),
...
start_replication(#rep_state{
    ...
    ?X_TRACE_IF(?x_trace(startReplication,
                         [{batchSizeItems, BatchSizeItems},
                          ...
                          {workers, {json, [xdcr_trace:format_pid(W) || W <- Workers]}}])),
        ...
        {get_changes, From} ->
            case couch_work_queue:dequeue(ChangesQueue, BatchSize) of
                ...
                {ok, Changes, _Size} ->
                    ...
                    ?X_TRACE_IF(?x_trace(sendingChanges,
        ...
        _ ->
            ...
            proc_lib:spawn_link(
              fun () ->
                      ...
                      ?X_TRACE_IF(?x_trace(gotNotification, [{rv, xdcr_trace:format_pp(RV)}])),
                      Self ! wake_me_up
              end)
    end.
