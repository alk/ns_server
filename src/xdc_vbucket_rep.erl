...
handle_call({report_seq_done,
    ...
    ?xdcr_extra_debug("Replicator of vbucket ~p: worker reported seq ~p, through seq was ~p, "
...
handle_call({worker_done, Pid}, _From,
    ...
    case Workers -- [Pid] of
        ...
        [] ->
            ...
            ?xdcr_extra_debug("Replicator of vbucket ~p done, return token to throttle: ~p~n"
                 ...
                 replicating ->
                     ...
                     case xdc_vbucket_rep_ckpt:do_checkpoint(State) of
                         {ok, _, NewState} ->
                             ...
                             ?xdcr_extra_debug("checkpoint issued during replication for vb ~p, "
...
init_replication_state(#init_state{rep = Rep,
    ...
    ?xdcr_extra_debug("history log at src and dest: startseq: ~p, docs checked: ~p,"
...
start_replication(#rep_state{
    ...
    ?xdcr_extra_debug("changes reader process (PID: ~p) and manager process (PID: ~p) "
               ...
               _XMemRemote  ->
                   XMemSrvPid = case State#rep_state.xmem_srv of
                                    nil ->
                                        ...
                                        XMemSrv;
                                    Pid ->
                                        ?xdcr_extra_debug("xmem remote server already started (vb: ~p, pid: ~p)",
                                                          [Vb, Pid]),
                                        Pid
                                end,
                   ok = xdc_vbucket_rep_xmem_srv:connect(XMemSrvPid, Remote),
                   ok = xdc_vbucket_rep_xmem_srv:select_bucket(XMemSrvPid),
                   ?xdcr_extra_debug("xmem remote node connected and bucket selected "
    ...
    case Succ of
        ok ->
            ?xdcr_extra_debug("checkpoint at start of replication for vb ~p "
